...
teams = pd.read_excel('Poss2022TeamID.xlsx')
games = pd.merge(games, teams[['Team']], left_on='Opponent', right_on='Team', how='inner').drop('Team_y', axis=1)
...
ratings = pd.merge(ratings, teams[['Team', 'Conference', 'AdjEM', 'AdjO', 'AdjD']],
                   left_on='team', right_on='Team').drop('team', axis=1)
ratings = ratings[['Team', 'Conference', 'EFG', 'TOR', 'ORB', 'FTR', 'OEFG', 'OTOR', 'OORB', 'OFTR', 'EFGRk',
                   'TORRk', 'ORBRk', 'FTRRk', 'OEFGRk', 'OTORRk', 'OORBRk', 'OFTRRk', 'AdjEM', 'AdjO', 'AdjD']]
# %%
...
ratings.to_excel('FourFactorRatings2223.xlsx', index=False)
